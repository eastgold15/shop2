...
export function useDailyInquiryCounterDetail(id: string, enabled = !!id) {
  return useQuery({
    ...
    queryFn: () => api.get<any>(`/api/v1/daily-inquiry-counter/${id}`),
    enabled,
  });
}
  ...
  return useMutation({
    mutationFn: ({
      ...
      api.put<any, typeof DailyInquiryCounterContract.Update.static>(
        `/api/v1/daily-inquiry-counter/${id}`,
        data
      ),
...
export function useDeleteDailyInquiryCounter() {
  ...
  return useMutation({
    mutationFn: (id: string) =>
      api.delete<any>(`/api/v1/daily-inquiry-counter/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: dailyinquirycounterKeys.lists(),
      });
