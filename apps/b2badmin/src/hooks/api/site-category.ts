...
export function useSiteCategoryDetail(id: string, enabled = !!id) {
  return useQuery({
    ...
    queryFn: () => api.get<any>(`/api/v1/site-category/${id}`),
    enabled,
  });
}
...
export function useDeleteSiteCategory() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (id: string) => api.delete<any>(`/api/v1/site-category/${id}`),
    onSuccess: () => {
...
export function useBatchDeleteSiteCategories() {
  ...
  return useMutation({
    mutationFn: (ids: string[]) =>
      api.delete<any, { ids: string[] }>("/api/v1/site-category/batch", {
        ids,
      }),
    onSuccess: () => {
...
export function useToggleCategoryStatus() {
  ...
  return useMutation({
    mutationFn: (id: string) =>
      api.patch<any, {}>(`/api/v1/site-category/${id}/toggle`, {}),
    onSuccess: () => {
